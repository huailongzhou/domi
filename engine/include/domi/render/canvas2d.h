...
public:
    ...
    // Mutable texture support (used by Font's glyph atlas). The texture is
    // owned by the backend; it stays alive until destroyMaterial() is called
    // or the backend itself is destroyed.
    void* createMutableTexture(int width, int height);
    void updateTextureRegion(void* handle, int x, int y, int w, int h,
                             const void* rgbaPixels);

    // Draw a sub-region of a material/mutable texture with its top-left at
    // (x, y), tinted by a multiplicative color. Honors the current transform
    // and goes through the batching queue like other draws.
    void drawMaterialRegion(void* handle, int srcX, int srcY, int srcW, int srcH,
