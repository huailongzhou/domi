...
public:
    ...
    // Leaf nodes built during the last load, in DFS order (matches a DFS walk
    // of the JSON tree). Used by the editor to bind physics bodies to visuals.
    struct BuiltLeaf {
        std::string type; // "rect" | "ellipse" | "material"
        RenderNode* node;
        RectNode* rect;
        EllipseNode* ellipse;
        MaterialNode* material;
        BuiltLeaf()
            : node(NULL), rect(NULL), ellipse(NULL), material(NULL) {}
    };
    const std::vector<BuiltLeaf>& leaves() const { return leaves_; }
