...
class SceneFunction {
public:
    // Record a directional shadow on the ground as a soft ellipse.
    //
    // @param batch         target draw batch
    ...
    //                      when shadowDir.y <= 0 nothing is recorded
    static void drawShadow(DrawBatch& batch, float x, float y,
